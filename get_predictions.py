#***IMPORTANT NOTES!!***
#When scanning the JPEG files in simplex mode and passing 
#in the "simplex" argument when running the code, you
#cannot start your actual JPEG file names with "back",
...
if __name__ == '__main__':
    ...
    #The function "return_file_name_suffix_integer()" will extract any sequences 
    #of one or more digit characters preceding the ".jpg" file extension and return 
    #their integer-casted value. If there were no such digit sequences in the file name,
    #then the function will return zero, as the file name must then be the first file 
    #name without a numbered suffix ("my_file-.jpg" vs "my_file-0001.jpg").
    #This will allow the "sorted()" method to sort padded 
    #(e.g., "my_file-0001.jpg") or non-padded (e.g., "my_file-1") file names,
    #provided that the numbering pattern is uniform throughout the files.
    #This will prevent "my_file-10.jpg" from appearing before "my_file-2.jpg"
    #in the sorted list.
    def return_file_name_suffix_integer(file_name):
        digit_string_list = re.findall(r"([\d]+).jpg$", file_name)
        if digit_string_list != []:
            return int(digit_string_list[0])
        else:
            return 0
    ...
    if model_name != None:
        ...
        A4 = False
        #The "simplex_mode" variable, initialized to "False", will be set to "True" when 
        #the user passes in the "simplex" argument when running the code, which indicates 
        #that the JPEG images have been scanned in simplex mode. This means that the user 
        #has scanned all of the odd pages first (right-hand pages), then retrieved the 
        #stack of pages and submitted the even pages (left-hand pages) to be scanned next,
        #prefacing the file name of the left-hand pages with "back". The first left-hand 
        #side page to be scanned is then the flip side of the last right-hand page that 
        #was scanned, so when performing OCR, the pages will need to be interleaved by 
        #the code so that their sequence in the outputted RTF file matches the original 
        #sequence in which they were handwritten. Also, when segmenting the characters 
        #on the page, since the gutter margin needs to accomodate page perforations,
        #these will be larger than the outer margins, resulting in mirrored left and 
        #right margins on the left- and right-handed pages. The file names starting 
        #with "back" when in "simplex" mode will then be treated differently than the 
        #right-handed pages lacking the "back" prefix to their file names. In duplex 
        #mode (the default setting of "simplex_mode = False"), the user was instructed 
        #to orient the pages in the scanner such that the first JPEG file generated 
        #corresponds to the right-hand side of the first sheet that was scanned. 
        #This means that the even JPEG file indices in the list of JPEG file names 
        #that was sorted in ascending order will correspond to right-hand pages 
        #(i == 0, 2, 4, etc...), while even JPEG file indices will represent 
        #the left-hand-pages.
        simplex_mode = False
        ...
        if len(sys.argv) > 1:
            ...
            try:
                for i in range(1, len(sys.argv)):
                    ...
                    #(i == 0, 2, 4, etc...), while even JPEG file indices will represent 
                    #the left-hand-pages.
                    elif sys_argv_i_lower_strip[:7] == "simplex":
                        simplex_mode = True
                    #The default canvas size is set to 64x64 pixels,
                    #and the values of "default_pixel_width" and 
                    #"default_pixel_height" may be overriden by 
                ...
                print('\nThe following "Exception" was raised when running the code:', str(e))
                print("")
        ...
        #If the JPEG files were scanned in simplex mode ("simplex_mode == True"), then 
        #the user has first scanned the right-hand pages, retrieved the stack of pages 
        #and has then scanned the left-hand pages, where the first scanned left-hand 
        #page is the flip side of the last scanned right-hand page. As such, after 
        #sorting the JPEG file names in ascending order, the pages need to be interleaved 
        #by the code in order to generate OCR predictions in the same order than the 
        #pages were originally handwritten.
        if simplex_mode:
            #The list "JPEG_file_names" is populated with the ".jpg" file names in
            #the "Training&Validation Data" folder.The list is sorted in ascending order 
            #using the key provided by the function "return_file_name_suffix_integer()",
            #which returns the integer-casted value of the sequence of one or more digits 
            #preceding the ".jpg" file extension, or zero if there are no such sequences
            #in the file name. This will allow the "sorted()" method to sort padded 
            #(e.g., "my_file-0001.jpg") or non-padded (e.g., "my_file-1") file names,
            #provided that the numbering pattern is uniform throughout the files.
            #This will prevent "my_file-10.jpg" from appearing before "my_file-2.jpg"
            #in the sorted list.
            front_JPEG_file_names = ([file_name for file_name in sorted(os.listdir(os.path.join(cwd,
            "OCR Raw Data")), key=return_file_name_suffix_integer) if (file_name[:4].lower()!="back" and file_name[-4:] == ".jpg")])
            ...
            if front_JPEG_file_names not in [None, []]:
                OCR_text_file_name = re.findall(r"(.+?)[- _]*[\d]*.jpg$", front_JPEG_file_names[0])[0]
            ...
            if front_JPEG_file_names in [None, []] and back_JPEG_file_names in [None, []]:
                ...
                OCR_text_file_name = re.findall(r"(.+?)[- _]*[\d]*.jpg$", back_JPEG_file_names[0])[0]
        ...
        #If the JPEG files were scanned in duplex mode, then all of the pages (front and back)
        #were numbered successively, so no interleaving is requred and the file names are simply 
        #sorted in ascending order using the key provided by the function "return_file_name_suffix_integer()"
        else:
            #The list "JPEG_file_names" is populated with the ".jpg" file names in
            #the "Training&Validation Data" folder.The list is sorted in ascending order 
            #using the key provided by the function "return_file_name_suffix_integer()",
            ...
            "OCR Raw Data")), key=return_file_name_suffix_integer) if file_name[-4:] == ".jpg"])
            if JPEG_file_names in [None, []]:
                print('\nPlease include some JPEG scanned images of handwritten text in the "OCR Ras Data" folder. ' +
                'For best results, these should be scanned as JPEG images on a multipage scanner at a resolution of 300 dpi.')
                input("\nPress any key to continue")
                sys.exit()
            else:
                OCR_text_file_name = re.findall(r"(.+?)[- _]*[\d]*.jpg$", JPEG_file_names[0])[0]
        ...
        with open(os.path.join(path, OCR_text_file_name + ".txt"), 'w', encoding="utf-8") as f:
            ...
            def get_dot_x_coordinates(inches_between_dots, dot_diameter_pixels):
                ...
                #In simplex mode, if the page is even-numbered (the file name is starts with "back"
                #for "back page"), then the "starting_x" pixel will be initialized to "left_margin_x_pixel". 
                #The same will be done in duplex mode ("simplex_mode == False") when the JPEG file index 
                #in the list of JPEG file names that was sorted in ascending order is odd (i == 1, 3, 5, etc...), 
                #indicating that the image is of a left-hand page, as the first JPEG was for the right-hand of the 
                #first handwritten page.
                if simplex_mode and JPEG_file_names[i][:4].lower() == "back" or not simplex_mode and i%2 != 0:
            ...
            #A "try/except" statement is included here in case the user forgot to 
            #pass in the "simplex" argument when running the code on JPEG files that 
            #were scanned in simplex mode, as the code would then need to set the "simplex_mode"
            #Boolean varaible to "True" in order to process the left-hand pages (of whose 
            #file name start with "back") differently from the right-hand page during 
            #segmentation.
            try:
                for i in range(len_JPEG_file_names):
                    ...
                    #The "text" list is extended with the current page's list 
                    #of characters before moving on to the next page.
            ...
            #segmentation.
                ...
                #The RTF file is closed to allow deleting it and the folder it is in,
                #as we do not want to have an incomplete RTF file lying about.
                f.close()
                shutil.rmtree(path, ignore_errors=True)
                print('\nThe following "Exception" was raised when running the code:', str(e))
                print('\nIf you have scanned the JPEG files in simplex mode (one-sided scans), you will then need to pass in the "simplex" argument when running the code. Otherwise, if the JPEG files were scanned in duplex mode (two-sided scans), do not pass in the "simplex" argument when running the code.')
                input("\nPress any key to continue.")
                sys.exit()
            
            len_text = len(text)
            text_with_carriage_returns = []
            column_index = 0
            row_index = 0
            #The "for" loop below will cycle through all of the OCR characters for the "text" list 
            #and insert carriage returns ("\n") upon reaching the width of the canvas ("pixel_width") 
            #or after an "L" character, and a form feed page break ("\f") upon reaching the height 
            #of the canvas ("pixel_height") or after a "P" character. This will make the TXT files 
            #more human readable to correct OCR errors that pertain to "L" and "P" characters.
            for i in range(len_text):
                ...
                if text[i].lower() == "l":
                    if row_index == pixel_height - 2:
                        ...
                        column_index = 0
                    #If the current value of "row_index" isn't equal to the index of the last row ("pixel_height - 1"),
                    #as a page break would be inserted automatically, thus removing the need for a carriage return, and 
                    #either the "L" is the last character of the "text" list ("i == len_text - 1") or next character is 
                    #not a "P", then a carriage return ("\n") will be added after the current character at the index 
                    #"i" to the list "text_with_carriage_returns".
                    if (row_index < pixel_height - 1 and (i == len_text - 1 or 
                        i < len_text - 1 and text[i+1].lower() != "p")):  
                        text_with_carriage_returns += [text[i], "\n"]
                    #Otherwise, only the current character at the index "i" will be added to the list, 
                    ...
                    #after the last line in the next iteration of the loop.
                        ...
                        text_with_carriage_returns += [text[i]]
                #If the current character at index "i" is a "P", 
                ...
                #"text_with_carriage_returns".
                    ...
                    column_index = 0
                    row_index = 0
                    text_with_carriage_returns += [text[i], "\f"]
                    #If all of the characters in the "text" list following the "P"
                    #at index "i" are spaces, then the "for" loop will be broken out of, as we 
                    #do not want to include these trailing spaces in the list 
                    #"text_with_carriage_returns", so as to avoid 
                    #generating blank images.
                    if i < len_text - 1 and all(char == " " for char in text[i+1:]):
                        break
                ...
                #carriage return ("\n") will be added after the character at index "i" to the list "text_with_carriage_returns" 
                #if this isn't the final row ("row_index < pixel_height - 1") and either if this is the last character in the "text" list, 
                #or if the next character isn't an "L" nor a "P", as a carriage return ("\n") or form feed page break ("\f"), respectively, 
                #would be added after these characters in the next iteration of the "for" loop.
                elif column_index >= pixel_width - 1:
                    column_index = 0
                    if (row_index < pixel_height - 1 and (i == len_text - 1 or 
                        i < len_text - 1 and text[i+1].lower() not in ["l", "p"])):  
                        row_index += 1
                        text_with_carriage_returns += [text[i], "\n"]
                    else:
                        text_with_carriage_returns += [text[i]]
                #If this isn't the last character in the row, and isn't a "P" nor an "L", then the value 
                #of "column_index" will be incremented, and the character at the index "i" will be added 
                #to the the list "text_with_carriage_returns".
                else:
                    column_index += 1
                    text_with_carriage_returns += [text[i]]
                #As the character at index "i" could be both the last character of the last row of the current image, 
                ...
                #added after the current character in the list "text_with_carriage_returns",
                #and the indices "column_index" and "row_index" will both be reset to zero, as we are 
                #starting a new image.
                if (text[i].lower() != "p" and row_index >= pixel_height - 1 and 
                (text[i].lower() == "l" or column_index >= pixel_width - 1)):
                    if (i == len_text - 1 or i < len_text - 1 and text[i+1].lower() != "p"):
                        column_index = 0
                        row_index = 0
                        text_with_carriage_returns += ["\f"]
                        #If all of the characters in the "text" list following the current
                        #character at index "i" are spaces, then the "for" loop will be broken out of, 
                ...
                #the current row isn't the last one ("row_index < pixel_height - 1") and the current character 
                #is either the last character in the "text" list or the next character isn't a "P" after the "if" 
                #statement above, in order to avoid inserting a form feed page break ("\f") after the "L" 
                #when the "L" is found on the penultimate row, where "row_index == pixel_height - 2".
                elif (text[i].lower() == "l" and row_index < pixel_height - 1 and 
                (i == len_text - 1 or i < len_text - 1 and text[i+1].lower() != "p")):  
                    row_index += 1
            
            text = "".join(text_with_carriage_returns) + f"({pixel_width}x{pixel_height})"           
