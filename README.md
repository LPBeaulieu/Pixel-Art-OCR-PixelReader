...
In the image above, you can notice that the handwritten page cotains a flattened version (a continuous string of text) of the OCR text file. While the digits 0-9 and "space" characters (empty cells) encode the color of the pixels associated with these characters, the "L" and "P" characters have a different function. These latter characters designate line ("L") and page ("P") breaks, where a line break fills all the remaining cells up to the width of the pixel art canvas with spaces, and the page breaks signify that a new pixel art image is about to start. If your row already contains a full complement of characters like the fifth row in the first 16x16 pixel image (1444444444444441), which already contains 16 characters, then you do not need to add an "L" character to indicate that you will now be encoding the sixth row of pixels, because the code keeps track and automatically moves to the next line for you. However, I find that adding the "L" characters helps me check for mistakes after completing each row of pixels, as I can easily locate the start of the row and count the number of characters. You really do want to check your work at every row when handwriting the numbers, as each cell will make its way in your image, mistake or not. Notice that the first line of the text file only contains an "L" in the text file. That is because the "L" character tells the code that the remaining cells in the line are all comprised of "space" characters (here white pixels, as can be seen in the PNG file). Also notice the 16th line of the text file (last row of pixels of the frog image) only contains an "L", followed by a "P". The "L" is required to tell the code that all of the pixels in that row are white ("space" characters here), and the "P" indicates that a page break should be inserted. The "P" character isn't tied to any color and therefore must be placed *after* the "L" to insert a page break at the *end* of the row.<br><br>
...
While OCR errors resulting in a digit being changed to another digit may be best handled by modifying the PNG files directly, some OCR mistakes involving "L" or "P" characters will result in the loss of a line ("L") or page ("P") break if these characters get swapped for a digit or a space, for instance. In the example above, the topmost frog is the ideal frog, while the frog below it is less so. While the final character in row 2 is normally an "L", signifying a line break that will fill up the remaining cells of the row with spaces (here two more spaces will be added), the bottom frog has an OCR error that changed that "L" for a "1". Consequently, the code "ate up" the first space of the next line to make up for the missing character on the line, which ends up making rows 2 and 3 look staggered. To correct this, you would need to revert the erroneous 1 back to an "L", followed by deleting the extra space at the end of row 2 and adding a space 
at the start row 3.<br><br>
...
<b>File 2: "txt2png.py"</b>- This code will proofread your TXT files for common issues like rows containing too many characters (a number of characters exceeding your pixel art canvas width) and then generate your PNG file by converting each digit to its corresponding encoded color found in the "color_key.csv" Comma-Separated Value (CSV) file. 
