...
if __name__ == '__main__':
    ...
    if txt_file_paths:
        ...
        error_string = textwrap.fill("Please add a text file that was generated by the \"get_predictions_Win10_x86_64\" executable to the \"OCR Predictions\" folder and run the code again.", width=columns)
        print("")
        print(error_string)
            ...
            if len(row) > 1 and row[1] != "":
                ...
                #- 8-digit hex code with 2 alpha digits at the end
                    ...
                    if hex_search_result:
                        ...
                        #and rows in the console, to allow to properly format the text and dividers.
                        columns, lines = get_terminal_dimensions()
                        error_string = textwrap.fill(f"There was a problem while processing the following color mapping in your CSV file:{str(row)[1:-1]}\n", width=columns)
                        print("")
                        print(error_string)
                        input("\nPress any key to continue.")
                        sys.exit()
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            #and rows in the console, to allow to properly format the text and dividers.
            columns, lines = get_terminal_dimensions()
            exception_string = 'The following "Exception" was raised when running the code:' + str(e)
            exception_string = textwrap.fill(exception_string, width=columns)
            error_string = textwrap.fill(resolution_error_string, width=columns) 
            print("")
        ...
        try:
            for j in range(len(file_string_lines)):
                ...
                if len(file_string_lines[j].lower().replace("l", "").replace("p", "")) > pixel_width:
                    ...
                    #and rows in the console, to allow to properly format the text and dividers.
                    columns, lines = get_terminal_dimensions()
                    error_string = textwrap.fill(f"There are too many characters in row {j+1} of the text file \"{txt_file_names[i]}\" (pixel art image number {pixel_art_image_index}). Please examine the TXT file and correct the issues with this image and run this code again.", width=columns)
                    print("")
                    print(error_string)
                    input("\nPress any key to continue")
                    sys.exit()
            ...
            #and rows in the console, to allow to properly format the text and dividers.
            columns, lines = get_terminal_dimensions()
            error_string = textwrap.fill(f"There was a problem while processing the pixel art image number {pixel_art_image_index} of the text file \"{txt_file_names[i]}\". Please examine the TXT file and correct the issues with this image and run this code again.", width=columns)
            print(error_string)
            input("\nPress any key to continue")
            sys.exit()
    ...
    input("Your PNG files have successfully been created! Press any key to continue.")